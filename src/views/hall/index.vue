<template>
  <ScrollPage ref="ScrollPage" :pull-down="true" @refresh="pullDownrefresh" @loading="pullUploading" :pull-Up="false">
    <template #pullDown="{ state }">
      <PullDownSlot :state="state" />
    </template>
    <div class="viewContent">
      <div class="searchBar">
        <Field v-model="search" class="field" placeholder="搜索" disabled @click="goSeach">
          <template #label>
            <i class="iconfont">&#xe632;</i>
          </template>
        </Field>
      ...
      <ScrollBox title="最新专辑"  direction="x"  height="160px" :update="albumList.length">
        <template #rightBtn>
          <span>更多</span>
        </template>
        <template #content>
